...
void Game::updateDebugState()
{
	...
	bool has_basic_debug = has_debug || (player->hud_flags & HUD_FLAG_BASIC_DEBUG) || (g_settings->getBool("cheats") && g_settings->getBool("privilege_override"));
	...
	// noclip
	draw_control->allow_noclip = (m_cache_enable_noclip && client->checkPrivilege("noclip")) || (g_settings->getBool("cheats") && g_settings->getBool("freecam"));
}
...
{
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (!(client->checkPrivilege("debug") || (player->hud_flags & HUD_FLAG_BASIC_DEBUG) || (g_settings->getBool("cheats") && g_settings->getBool("privilege_override")))) {
		...
		const CameraOrientation &cam)
{
	...
	*/
	...
	if ((draw_control->allow_noclip && m_cache_enable_free_move &&
		client->checkPrivilege("fly"))|| (g_settings->getBool("freecam") && g_settings->getBool("cheats")) ) {
